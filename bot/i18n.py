...
TEXTS: dict[str, dict[str, str]] = {
    "ru": {
        ...
        # --- Welcome в группе (при вступлении) ---
        "group_welcome_unreg": (
            "👋 <a href=\"tg://user?id={user_id}\">{name}</a>, "
            "добро пожаловать в <b>{community}</b>!\n\n"
            "🔒 Пока ты не можешь писать в чате — нужно пройти регистрацию "
            "(займёт 1 минуту).\n\n"
            "Открой бота, выбери роль <b>Coworker</b> или <b>Заказчик</b> "
            "и сразу станешь полноценным участником:\n"
            "👉 @{bot_username} → /register"
        ),
        "group_welcome_unreg_blast": (
            "👋 <b>Для тех, кто ещё не зарегистрировался:</b>\n\n"
            "Если вы в группе, но не можете писать — нужна регистрация "
            "(займёт 1 минуту).\n\n"
            "Выберите роль <b>Coworker</b> (исполнитель) или "
            "<b>Заказчик</b> (ищу подрядчиков) и сразу сможете "
            "участвовать в чате, откликаться на объявления и размещать заявки.\n\n"
            "👉 Открой @{bot_username} и нажми /register"
        ),
        "admin_ghost_stats": (
            "👻 <b>«Призраки» — участники без регистрации</b>\n\n"
            "Всего в базе без роли (guest / без роли): <b>{ghost_count}</b>\n"
            "Из них удалённые профили: <b>{deleted_count}</b>\n"
            "Активные призраки (могут быть в группе): <b>{active_count}</b>\n\n"
            "Чтобы напомнить им о регистрации: /welcome_unreg"
        ),
        "admin_welcome_unreg_done": (
            "✅ Отправил в группу напоминание о регистрации.\n"
            "Призраков в базе: <b>{count}</b>"
        ),
        "admin_no_group_chat": (
            "⚠️ GROUP_CHAT_ID не настроен. Команда работает только если "
