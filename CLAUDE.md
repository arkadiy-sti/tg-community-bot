...
1. **Фидбэк-flow после Deal** *(самое ценное)* — когда Deal закрывается (или через X дней после), бот пингует обе стороны: ⭐ 1-5 + теги (positive/negative из существующего `feedback_pos/feedback_neg` словаря) + опц. комментарий. Заполняет `Feedback`+`FeedbackTag` (модели уже есть). Облако в `/profile` оживает, рейтинг считается.
2. **`/my_posts` + 14-дневный auto-expire** — список своих объявлений с быстрым «Закрыть». Шедулер раз в сутки помечает старые approved как expired. Аркадий подтвердил «раз в сутки достаточно».
3. **Подписки + декоратор** — `@subscription_required(plan='pro')`, Pro $20/мес физлица, Business $200/мес компании, ручной `/grant <user_id> <дней>` админом, потом Stripe.
